...
- On startup, `gghealthd` shall begin serving the `gg_health` Core Bus
  API interface.
...
- On request by `gg-fleet-statusd`, `gghealthd` shall report individual component lifecycle states. This will be used to report the overall device health as well as what components are installed on the device.
- `gghealthd` retrieves requested component information from `ggconfigd` for input validation.