...
See [log-trail design](../../design/log-trail.md) for architecture and usage
guidance.
...
- [tracing-macros-1] CMake option `-DGG_LOG_TRAIL=ON|OFF` (default OFF).
...
- [tracing-api-1]
  `void gg_log_set_trail(uint16_t trace_id, uint16_t span_id, uint16_t parent_span_id)` -
  sets the calling thread's trace context.
- [tracing-api-2] `void gg_log_clear_trail(void)` - clears the calling thread's
  trace context.
- [tracing-api-3] `uint16_t gg_log_current_trail_id(void)` - returns the active
  trace_id, or 0 if no trace is set.
- [tracing-api-4]
  `void gg_log_get_trail(uint16_t *trace_id, uint16_t *span_id, uint16_t *parent_span_id)` -
  reads the full trace context.

### Orchestration (priv_include/gg/log-trail.h)
...
- [tracing-api-6]
  `size_t gg_log_trail_attach_headers(EventStreamHeader *headers, size_t headers_capacity)` -
...
- [tracing-api-7]
  ...
  iterates inbound headers for T and S entries (type-guarded to INT32).
...
- [tracing-api-8] `GG_LOG_TRAIL_SCOPE_GUARD()` - declares a variable with
  `__attribute__((cleanup))` that calls `gg_log_clear_trail` on scope exit.
- [tracing-api-9] `GG_LOG_TRAIL_ROOT_SCOPE(kind, fmt, ...)` - defensive clear +
  `gg_log_trail_root_begin(...)` + `GG_LOG_TRAIL_SCOPE_GUARD()`. Self-gates:
  expands to nothing when `GG_LOG_TRAIL_ENABLED` is not defined.
