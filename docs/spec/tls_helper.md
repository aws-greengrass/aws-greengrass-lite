...
unix domain socket. Except in case of an error, the helper MUST use the the
...
TLS with the provided private key and certificate. It MUST validate the server
using the provided root CA. It MAY handle the private key or certificate values
...
after the helper exits (such as if the provided socket is the TCP socket with
kTLS enabled for both rx and tx), the helper MAY exit with a zero error code.
