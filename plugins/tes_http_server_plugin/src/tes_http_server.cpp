...
const char* get_tes_credentials() {
    ...
    auto tes_lpc_response = ggapi::Task::sendToTopic(ggapi::Symbol{"aws.greengrass.requestTES"}, tes_lpc_request);
...
int _on_request_done(struct aws_http_stream *stream, void *user_data) {
    ...
    struct aws_input_stream *response_body = aws_input_stream_new_from_cursor(aws_default_allocator(), &body_src);
...
int _on_request_headers_done(
    ...
    if(aws_http_headers_get(all_headers,tes_header_cursor, &authz_header_value) != AWS_OP_SUCCESS){
...
void _on_shutdown(aws_http_connection *connection,
                  int error_code,
...
void TesHttpServer::start_server() {
    ...
    // TODO: Revisit this to check if there a way to get the randomly assigned port number. For now, use 8080.
