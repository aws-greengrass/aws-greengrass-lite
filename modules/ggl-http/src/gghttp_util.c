...
typedef struct CurlRequestRetryCtx {
    ...
    // Needed to propogate non-retriable errors.
    GglError err;
} CurlRequestRetryCtx;

static GglError retry_reset_buffer(void *response_data) {
    GglByteVec *vector = (GglByteVec *) response_data;
    vector->buf.len = 0;
    return GGL_ERR_OK;
}

static GglError retry_reset_fd(void *response_data) {
...
static GglError curl_request_retry_wrapper(void *ctx) {
    ...
    if (curl_error != CURLE_OK) {
        GGL_LOGE(
            "Curl request failed due to non-retriable error: %s",
    ...
    GGL_LOGE(
        "Curl request failed due to non-retriable HTTP status code %ld.",
        http_status_code
    );
    return GGL_ERR_OK;
}
...
static GglError do_curl_request(
    ...
    CurlRequestRetryCtx ctx = { .curl_data = curl_data,
                                ...
                                .retry_fn = retry_reset_buffer,
                                .err = GGL_ERR_OK };
    GglError ret = ggl_backoff(
        1000,
        1000 * MAX_RETRY_WAIT_SEC,
        MAX_RETRIES,
        curl_request_retry_wrapper,
        (void *) &ctx
    );
    if (ret != GGL_ERR_OK) {
        return ret;
